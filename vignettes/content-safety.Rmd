...
```{r moderate-summary-rendered, echo = FALSE, eval = run_api && requireNamespace("gt", quietly = TRUE)}
results |>
...
```{r moderate-severity-data, include = FALSE, eval = run_api && requireNamespace("ggplot2", quietly = TRUE)}
text_on <- function(fill) {
...
wrap_label <- function(x, width = 24) {
  vapply(
    x,
    function(value) paste(strwrap(value, width), collapse = "\n"),
    character(1),
    USE.NAMES = FALSE
  )
}
...
review_threshold <- 2
severity_palette <- grDevices::colorRampPalette(c("#F3F2F1", "#A4262C"))(8)
severity_grid <- results |>
  dplyr::mutate(
    text = factor(text, levels = rev(unique(text))),
    fill = severity_palette[pmin(pmax(severity, 0), 7) + 1],
    tile_label = paste(severity, label, sep = "\n")
  )
stopifnot(!anyNA(severity_grid$text), !anyNA(severity_grid$fill))

n_texts <- nlevels(severity_grid$text)
flagged <- severity_grid[severity_grid$severity >= review_threshold, ]
n_flagged <- length(unique(flagged$text))
severity_chart_title <- if (n_flagged == 0) {
  sprintf("None of the %d texts reached the review threshold", n_texts)
} else {
  sprintf("%d of %d texts reached the review threshold", n_flagged, n_texts)
}
severity_chart_alt <- paste0(
  severity_chart_title,
  ". Grid of Azure AI Content Safety severity scores for ",
  n_texts,
  " sample texts across the ",
  paste(sort(unique(severity_grid$category)), collapse = ", "),
  " categories. ",
  if (n_flagged == 0) {
    "Every tile shows severity 0 (safe)."
  } else {
    paste0(
      "Tiles at or above severity ",
      review_threshold,
      ": ",
      paste(sprintf("%s in %s (%d)", flagged$text, flagged$category, flagged$severity), collapse = "; "),
      "."
    )
  }
)
...
```{r moderate-severity-chart, echo = FALSE, eval = run_api && requireNamespace("ggplot2", quietly = TRUE), fig.alt = get0("severity_chart_alt", ifnotfound = "Grid of Content Safety severity scores by text and harm category.")}
ggplot2::ggplot(severity_grid, ggplot2::aes(x = category, y = text)) +
  ggplot2::geom_tile(ggplot2::aes(fill = fill), color = "white", linewidth = 1.2) +
  ggplot2::geom_text(
    ggplot2::aes(label = tile_label, color = text_on(fill)),
    size = 3.4,
    lineheight = 0.95
  ) +
  ...
  ggplot2::labs(
    title = wrap_text(severity_chart_title, 62),
    subtitle = wrap_text(
      c(
        "Severity by text and harm category (0 safe, 2 low, 4 medium, 6 high)",
        sprintf("Texts at severity %d or more need review", review_threshold)
      ),
      84
    ),
    x = NULL,
    y = NULL,
    caption = wrap_text(
      "foundry_moderate() results for the three sample texts above. Tiles darken as severity rises.",
      101
    )
  ) +
  ...
  ggplot2::theme(
    ...
    axis.text.x.top = ggplot2::element_text(face = "bold", color = "grey15"),
    axis.text.y = ggplot2::element_text(color = "grey15", hjust = 0)
  )
```
...
```{r moderate-threshold, eval = run_api && requireNamespace("tidyr", quietly = TRUE)}
library(dplyr)
library(tidyr)
...
filtered result is empty. With real user content, rows that need review appear
here.
...
Combine all three safety features in a helper. Defining the function is local;
calling it requires configured Azure services and is not done here:

```{r safe-pipeline, eval = TRUE}
library(dplyr)
