...
The rendered evaluation outputs below use precomputed, illustrative data from a
representative resampling run. They are rendered during the documentation build
without Azure credentials.

```{r tidymodels-metrics-rendered, eval = TRUE, echo = FALSE}
tidymodels_cache <- readRDS(foundryr_vignette_cache("tidymodels.rds"))

tidymodels_cache$metrics |>
  ...
  gt::tab_header(title = "Embedding classifier evaluation metrics") |>
  gt::tab_options(table.font.names = "Inter")
```

```{r tidymodels-roc-rendered, eval = TRUE, echo = FALSE, fig.alt = "ROC curve for a cached embedding classifier evaluation."}
ggplot2::ggplot(
  tidymodels_cache$roc,
  ...
  ggplot2::geom_line(color = "#0078D4", linewidth = 1.1) +
  ggplot2::geom_point(color = "#0078D4", size = 2.2) +
  ggplot2::coord_equal() +
  ggplot2::labs(
    title = "ROC curve for a classifier built on cached embeddings",
