...
```{r basic-recipe, eval = have_tidymodels}
library(tidymodels)
library(foundryR)
...
```{r dimensions, eval = have_tidymodels}
recipe_spec <- recipe(sentiment ~ text, data = reviews) %>%
  step_foundry_embed(
    text,
    model = "text-embedding-3-small",
...
```{r multi-column, eval = have_tidymodels}
...
```{r keep-original, eval = have_tidymodels}
recipe_spec <- recipe(sentiment ~ text, data = reviews) %>%
  step_foundry_embed(
    text,
    model = "text-embedding-3-small",
...
```{r troubleshoot, eval = have_tidymodels}
