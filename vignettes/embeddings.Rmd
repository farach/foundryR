...
similarity_chart_title <- if (min(same_scores) > max(cross_scores)) {
  "Same-source pairs outscore every cross-source pair"
} else {
  "Same-source pairs tend to outscore cross-source pairs"
}
  ...
  ggplot2::labs(
    ...
    caption = wrap_text(
      c(
        ...
        "Scores from foundry_similarity() on text-embedding-3-small embeddings."
      ),
      101
    )
  ) +
...
ggplot2::ggplot(
  ...
  ggplot2::labs(
    ...
    caption = wrap_text(
      c(
        "Embeddings from text-embedding-3-small via foundry_embed().",
